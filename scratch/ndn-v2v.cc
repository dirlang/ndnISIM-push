...
NdnV2VTest::InstallBackground(int nPair, int frequency)
{
    ...
    int min = 1.0;
    int max = m_nNodes -1 ;
    rn->SetAttribute ("Min", DoubleValue (min));
    rn->SetAttribute ("Max", DoubleValue (max));


    //std::set<int> nodeSet; // set of candiate nodes
    std::set<int> consumerSet; // set for consumers
    //std::set<int> producerSet; // set for producers
    ...
    int consumerId = rn->GetInteger();
    
    for (int j=0; j<nPair; j++) {
        ret = consumerSet.insert(consumerId);

        while (! ret.second) {
             consumerId = rn->GetInteger(); // next consumer Id
             ret = consumerSet.insert(consumerId);
         }
...
int main (int argc, char *argv[])
{
  ...
  bool onCCH = false; // default not CCH
  ...
  if (onCCH) {
     cout << "Sending Interest on WAVE CCH" << endl;
  }
  else {
     cout << "Sending Interest on WAVE SCH" << endl;
  }
  ...
  exp.Start(nNodes/2, frequency); // number of background sessions; background Interest frequency 
